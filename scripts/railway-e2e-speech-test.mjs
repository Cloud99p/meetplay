// END-TO-END speech test against the LIVE Railway deployment:
// synthesize real speech with Deepgram TTS, stream PCM through
// wss://meetplay-production.up.railway.app/api/stt (the exact path the
// deployed browser client uses), verify words come back as Results.
...
const text = "Hello from MeetPlay, this is a caption test on Railway.";
const STT_URL = process.env.STT_URL || "wss://meetplay-production.up.railway.app/api/stt";
...
console.log(`2. Streaming through ${STT_URL}...`);
...
ws.on("message", (data) => {
  ...
  if (msg.type === "Metadata") console.log(`+${Date.now() - t0}ms Metadata (session up)`);
  ...
  if (msg.type === "Error") console.log("server error:", JSON.stringify(msg).slice(0, 300));
});

ws.on("close", () => {
  if (words) {
    console.log(`\n✅ WORDS PICKED UP ON RAILWAY (${Date.now() - t0}ms): "${words}"`);
    process.exit(0);
  } else {
    console.log("\n❌ NO WORDS RETURNED (session connected but no transcript)");
    process.exit(2);
  }
});
ws.on("error", (e) => { console.error("ws error:", e.message); process.exit(3); });
setTimeout(() => { console.log("TIMEOUT 25s"); try { ws.close(); } catch {} process.exit(1); }, 25000);
