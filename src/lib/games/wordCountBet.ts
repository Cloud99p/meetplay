...
/** Most frequent non-stopword (min 3 chars) across utterances. */
export function selectTargetWord(utterances: UtteranceLike[]): string {
...
/** Count occurrences — case-insensitive, substring match ('roadmapping' hits 'roadmap'). */
export function countOccurrences(word: string, utterances: UtteranceLike[]): number {
...
/** Winning guess = closest to the correct count (ties keep the first). */
...
/** Score a bet: max 1000 for exact, scales down by absolute difference. */
export function calculateBetScore(guess: number, actualCount: number): number {
  ...
  if (delta <= 3) return Math.round(1000 - delta * 100);
  if (delta <= 10) return Math.round(1000 - 300 - (delta - 3) * 30);
